                ...
                <section className="card-surface mt-8 p-6">
                    ...
                    <dl className="mt-4 grid gap-3 sm:grid-cols-2">
                        {[
                            ...
                            ['Perusahaan', transaction.member?.company || '-'],
                            ['Partner', transaction.partner?.name],
                            ['Kategori', transaction.partner?.category],
                    ...
                    <div className="mt-6 rounded-xl border border-ink/10 p-4">
                        <div className="flex justify-between text-sm"><span>Total Belanja</span><span className="font-semibold">{formatRupiah(transaction.total_amount)}</span></div>
                        <div className="mt-1 flex justify-between text-sm text-sage"><span>Diskon</span><span>-{formatRupiah(transaction.discount_amount)}</span></div>
                        <div className="mt-2 flex justify-between border-t border-ink/10 pt-2 font-display text-lg font-bold">
                            <span>Penjualan Bersih</span><span>{formatRupiah(transaction.net_amount)}</span>
                        </div>
                    </div>

                    {transaction.proof_url && (
                        <div className="mt-4">
                            <a href={transaction.proof_url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-gold-deep">Lihat foto struk →</a>
                        </div>
                    )}
                </section>

                <div className="mt-8">
                    <Link href={route('admin.transactions.index')} className="text-sm font-medium text-gold-deep">← Kembali</Link>
                </div>
            </div>
        </>
    );
