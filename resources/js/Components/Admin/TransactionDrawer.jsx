                ...
                <section className="rounded-2xl border border-ink/10 p-5">
                    ...
                    <dl className="mt-3 grid gap-3 sm:grid-cols-2">
                        {[
                            ...
                            ['Perusahaan', t.member?.company || '-'],
                            ['Partner', t.partner?.name],
                            ['Kategori', t.partner?.category],
                            ['Promo', t.promo?.title || '-'],
                            ['Catatan', t.note || '-'],
                        ].map(([label, value]) => (
                    ...
                    <div className="mt-5 rounded-xl border border-ink/10 p-4">
                        <div className="flex justify-between text-sm"><span>Total Belanja</span><span className="font-semibold">{formatRupiah(t.total_amount)}</span></div>
                        <div className="mt-1 flex justify-between text-sm text-sage"><span>Diskon</span><span>-{formatRupiah(t.discount_amount)}</span></div>
                        <div className="mt-2 flex justify-between border-t border-ink/10 pt-2 font-display text-lg font-bold">
                            <span>Penjualan Bersih</span><span>{formatRupiah(t.net_amount)}</span>
                        </div>
                    </div>

                    {t.proof_url && (
                        <div className="mt-4">
                            <a href={t.proof_url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-gold-deep">Lihat foto struk →</a>
                        </div>
                    )}
                </section>
            </div>
